...
class StorageHelperGenConverter {
  ...
  bool? getBoolValue(DartObject? obj, String name) =>
      obj?.getField(name)?.toBoolValue() ?? false;

  /// Takes the object "[obj]" as a parameter and the name of the attribute to read the value of
  /// Return the attribute as int
  int? getIntValue(DartObject? obj, String name) =>
      obj?.getField(name)?.toIntValue();

  /// Takes the object "[obj]" as a parameter and the name of the attribute to read the value of
  /// Return the attribute as a double
  double? getDoubleValue(DartObject? obj, String name) =>
      obj?.getField(name)?.toDoubleValue();
  ...
  Map<DartObject?, DartObject?>? getMapValue(DartObject? obj, String name) =>
      obj?.getField(name)?.toMapValue();

  /// Given a list of `DartObject` returns a list of` T` by converting each element
  List<T?>? getList<T>(List<DartObject?>? listObject) =>
      listObject?.map((DartObject? obj) => convert<T>(obj)).toList();
  ...
  Map<K?, V?>? getMap<K, V>(Map<DartObject?, DartObject?>? mapObject) {
    ...
    for (int i = 0; i < (keys?.length ?? 0); i++) {
      // For each key
      map[keys![i]] = convert<V>(
          mapObject![origKeys![i]]!); //  Convert key and convert value
    }
  ...
  T? convert<T>(DartObject? obj) {
    storageHelperLog("Convert from \"${T.toString()}\"");
      ...
      switch (T.toString()) {
        ...
        case 'StorageHelperCategory?':
          ...
          return StorageHelperCategory(
              ...
              elements:
                  getList<StorageHelperElement?>(getListValue(obj, "elements"))
                      as List<StorageHelperElement?>,
              addSource: getStringValue(obj, "addSource")) as T;
        case 'StorageHelperElement':
        case 'StorageHelperElement?':
        case 'StorageHelperElement<dynamic>?':
        case 'StorageHelperElement<dynamic>':
          ...
          List<String?>? concateneKeys =
              getList<String>(getListValue(obj, "concateneKeys"));
          List<String?>? concateneKeysFromArgument =
              getList<String>(getListValue(obj, "concateneKeysFromArgument"));
          List<String?>? description =
              getList<String>(getListValue(obj, "description"));
          ...
          String? defaultValueToString =
              obj?.getField("defaultValue")?.toString();

          if (typeToString?.contains("StorageHelperType") ?? false) {
            // If it is a StorageHelperType
            ...
            try {
              ...
              String? index =
                  split?[1]?.replaceAll("int (", "").replaceAll(")", "");
              type = StorageHelperType
                  .values[int?.tryParse(index as String) as int];
            } catch (e) {
              try {
                ...
                throw new StorageHelperException(
                    "Unable to identify StorageHelperElement's type");
              }
            }

            try {
              if (defaultValueToString?.contains("bool?") ?? false) {
                // Se è un booleano
                defaultValue = defaultValueToString
                        ?.substring(0, defaultValueToString.length - 1)
                        .replaceAll("bool? (", "") ==
                    "true";
              } else if (defaultValueToString?.contains("int?") ?? false) {
                // Se è un intero
                defaultValue = int?.tryParse(defaultValueToString
                    ?.substring(0, defaultValueToString.length - 1)
                    .replaceAll("int? (", "") as String);
              } else if (defaultValueToString?.contains("double?") ?? false) {
                // Se è un double
                defaultValue = double?.tryParse(defaultValueToString
                    ?.substring(0, defaultValueToString.length - 1)
                    .replaceAll("int? (", "") as String);
              } else if (defaultValueToString?.contains("DateTime?") ?? false) {
                // Se è un DateTime
                defaultValue = DateTime?.parse(defaultValueToString
                    ?.substring(0, defaultValueToString.length - 1)
                    .replaceAll("DateTime? (", "") as String);
              } else if (defaultValueToString?.contains("String?") ?? false) {
                // Se è un String
                defaultValue = defaultValueToString
                    ?.substring(0, defaultValueToString.length - 1)
                    .replaceAll("String? (", "");
                defaultValue = defaultValue?.substring(
                    0,
                    defaultValue.length -
                        1); // RImuovo l'ultimo carattere (apice)
                defaultValue = defaultValue
                    ?.substring(1); // Rimuovo il primo carattere (apice)
              } else if (defaultValueToString?.contains("bool") ?? false) {
                // Se è un booleano
                defaultValue = defaultValueToString
                        ?.substring(0, defaultValueToString.length - 1)
                        .replaceAll("bool (", "") ==
              ...
              throw new StorageHelperException(
                  "Unable to get default value of element with key \"§key\§");
            }
          } else {
          ...
          if (type == null)
            throw new StorageHelperException(
                "Unable to get type of element with key \"§key\§");
        ...
        case 'StorageHelperModel?':
          ...
          StorageHelperModel model = StorageHelperModel(
              categories: getList<StorageHelperCategory?>(
                      getListValue(obj, "categories"))
                  as List<StorageHelperCategory?>,
              log: getBoolValue(obj, "log"),
              dateFormat: getStringValue(obj, "dateFormat"));

          storageHelperLog("Model successfully converted!");
